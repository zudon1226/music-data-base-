...
type Tab = "library" | "upload" | "analytics" | "sponsors" | "stage";
...
export default function Home() {
  ...
  useEffect(() => {
    setUploads(JSON.parse(localStorage.getItem("z-v16-uploads") || "null") || starterUploads);
  }, []);

  useEffect(() => {
    localStorage.setItem("z-v16-uploads", JSON.stringify(uploads));
  }, [uploads]);
  ...
  const topArtist = uploads[0]?.artist || "No Artist Yet";
  ...
  return (
    <div style={page}>
      ...
      <main style={main}>
        {plan === "free" && (
          <div style={adBanner}>SPONSOR AD — V16 Free Version. Upgrade to Premium to remove ads.</div>
        )}

        <h1 style={titleStyle}>Music Business Platform V16</h1>
        ...
        {tab === "upload" && (
          <section style={uploadForm}>
            <h2>V16.1 Real Upload System</h2>
        ...
        {tab === "stage" && (
          <section style={stage}>
            <h1>V16.3 Artist Stage</h1>
            <h2>{topArtist}</h2>
            <p>Featured artist performance space.</p>
            <p>Upload music videos, live visuals, and artist showcases here.</p>
