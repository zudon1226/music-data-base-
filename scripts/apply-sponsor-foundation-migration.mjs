/**
 * Apply sponsor foundation migration (202609101001).
 * Usage: node scripts/apply-sponsor-foundation-migration.mjs
 */
...
const migrationFile = "202609101001_sponsor_foundation.sql";
...
try {
    ...
    console.log(`PASS applied ${migrationFile}`);
} catch (error) {
    const message = String(error?.message || error);
    if (/already exists|duplicate/i.test(message)) {
        console.log(`PASS ${migrationFile} (already applied)`);
    } else {
        console.error(`FAIL ${migrationFile}: ${message.slice(0, 300)}`);
        process.exitCode = 1;
    }
} finally {
    await client.end();
...
console.log("\nSponsor foundation migration complete.");
